$(document).ready(function(){
 ...
 var url = "/admin/maintenance/year";
 var id='';
 var url2 = "/admin/maintenance/year/checkbox";
 ...
 $('#add_year').on('hide.bs.modal', function(){
  $('#frmYear').parsley().destroy();
  $('#frmYear').trigger("reset");
});
 $('#year-list').on('change', '#isActive',function(){ 
 ...
 function refresh(){
  ...
  function(isConfirm) {
    ...
    $('#year-list').on('click', '.open-modal',function(){ 
      var link_id = $(this).val();
      id = link_id;
      $.get(url + '/' + link_id + '/edit', function (data) {
        ...
        if(data=="Deleted"){
          ...
          $('h4').text('Edit Year');
          $('#strYearDesc').val(data.description);
          $('#btn-save').val("update");
          $('#add_year').modal('show');
        }
      })
    ...
    $('#year-list').on('click', '.btn-delete',function(){ 
     var link_id = $(this).val();
     swal({
      title: "Are you sure?",
      type: "warning",
    ...
    $("#btn-save").click(function () {
      ...
      if($('#frmYear').parsley().isValid())
      {
        ...
        var formData = {
          strYearDesc: $('#strYearDesc').parsley('data-parsley-whitespace','squish').getValue()
        }
